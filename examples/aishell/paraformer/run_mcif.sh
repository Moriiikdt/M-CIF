...
config=paraformer_conformer_12e_12d_2048_256_cifout_linear.yaml #your config file name
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    --config-name "${config}" \
    ++train_data_set_list="/train_all.jsonl" \
    ++cmvn_file="/aishell/DATA/data/cmvn.json"
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    python utils/whisper_text2token.py --output ${token_list} -scale medium
    # echo "<blank>" > ${token_list}
    # echo "<s>" >> ${token_list}
    # echo "</s>" >> ${token_list}
    # utils/text2token.py -s 1 -n 1 --space "" ${feats_dir}/data/$train_set/text | cut -f 2- -d" " | tr " " "\n" \
    #     | sort | uniq | grep -a -v -e '^\s*$' | awk '{print $0}' >> ${token_list}
    echo -e "<unk>" >> ${token_list}
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  log_file="${exp_dir}/exp/${model_dir}/train.log.txt.${current_time}.log"
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  for dset in ${test_sets}; do
    ...
    echo "Computing WER ..."
    python utils/postprocess_text_en.py ${inference_dir}/1best_recog/text ${inference_dir}/1best_recog/text.proc
    python utils/postprocess_text_en.py  ${data_dir}/text ${inference_dir}/1best_recog/text.ref
