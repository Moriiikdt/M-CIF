...
class ParaformerSANDecoder(BaseTransformerDecoder):
    ...
    def forward(
        ...
        """
        tgt = ys_in_pad
        # print("ys_in_pad",ys_in_pad.shape)
        # print("ys_in_lens",ys_in_lens.shape)
        # print(f"Current batch max target length: {max(ys_in_lens)}, tgt.size(1): {tgt.size(1)}")
        tgt_mask = (~make_pad_mask(ys_in_lens,maxlen=tgt.size(1))[:, None, :]).to(tgt.device)
        ...
        for layer_id, decoder in enumerate(self.decoders):
            # print("decoder_tgt_mask",tgt_mask.shape)
            # print("decoder_memory",memory.shape)
            # print("decoder_memory_mask",memory_mask.shape)
