...
import torch,os,json,random,time
import logging
import numpy as np
...
@tables.register("predictor_classes", "MultiScaleCifPredictor")
class MultiScaleCifPredictor(torch.nn.Module):
    def __init__(self, idim, l_order, r_order,threshold=1.0, dropout=0.1,
                 smooth_factor=1.0, noise_threshold=0, tail_threshold=0.45,
                 ctc_weight=1, vocab_size=10000,**kwargs):
        super().__init__()
        ...
        # multi CIF
        self.scale_predictors = torch.nn.ModuleList([
            CifPredictor(
                idim, l_order, r_order, threshold, dropout,
                smooth_factor, noise_threshold, tail_threshold
            ) for _ in range(3)
        ])

        # CTC moudule
        self.scale_ctcs = torch.nn.ModuleList([
            CTC(
                odim=vocab_size,
                encoder_output_size=idim,
                reduce=True
            ) for _ in range(2)
        ])

        # MAE
        self.mae_criterion = mae_loss()

        positionwise_layer = PositionwiseFeedForward

        # linear
        self.m_cif = torch.nn.ModuleList([
           torch.nn.Linear(idim, idim) for _ in range(2)
        ])

        # after linear norm
        self.layer_norms = torch.nn.ModuleList([
            LayerNorm(idim, dim=-1)  
            for _ in range(3)
        ])


    def forward(self, hidden, target_label=None,mask=None,ignore_id=-1, **kwargs):
        
        mae_loss = 0.0
        mae_list=[None,None]
        ctc_total_loss=0.0
        ctc_list=[None,None]
        alpha_smooth_loss = 0.0
        B, T, D = hidden.size()
        device = hidden.device

        if target_label is not None:
            scale_targets =  target_label

        weighted_hidden = hidden if self.training else hidden
        
        current_mask = mask

        final_outputs = (None, None, None, None)
        ...
        for scale_idx in range(3):
            ...
            if target_label is not None:
                processed_target = scale_targets[scale_idx].to(device)

                target_lens = (processed_target != ignore_id).sum(dim=1).to(torch.int32)

                T_new = weighted_hidden.size(2)
                encoder_lens = torch.full((B,), T_new, device=weighted_hidden.device, dtype=torch.int32)

                if scale_idx == 2:
                    sec_emb=(weighted_hidden,encoder_lens)

            # CTC loss
            if target_label is not None and scale_idx != 2:
                scale_ctc_loss = self.scale_ctcs[scale_idx](
                        weighted_hidden, 
                        encoder_lens,
                        processed_target,
                        target_lens
                ...
                ctc_list[scale_idx]=scale_ctc_loss
                ctc_total_loss += scale_ctc_loss

            # CIF process
            base_embeds, token_num, scale_alpha, cif_peak, mask_cif = self.scale_predictors[scale_idx](
                weighted_hidden,
                target_label=processed_target if target_label is not None else None,
                mask=current_mask,
                ignore_id=ignore_id,
                **kwargs
            )
            ...
            # linear 
            if scale_idx != 2:
                base_embeds=self.m_cif[scale_idx](base_embeds)

            base_embeds = self.layer_norms[scale_idx](base_embeds)

            weighted_hidden = base_embeds
            current_mask = mask_cif
            final_outputs = (base_embeds, token_num, scale_alpha, cif_peak)

            if target_label is not None and scale_idx != 2:
                with torch.no_grad():
                    target_lengths = (processed_target != ignore_id).sum(dim=1).float()
                mae_list[scale_idx]=self.mae_criterion(token_num, target_lengths)
                mae_loss += mae_list[scale_idx]

        return final_outputs,ctc_total_loss,mae_loss,ctc_list,mae_list,sec_emb
    ...
    def forward(
        ...
        with autocast(False):
            ...
            if mask is not None:
                ...
                assert alphas.size(1) == mask.size(1), f"error: {alphas.shape} vs {mask.shape}"
                alphas = alphas * mask
                mask = mask.squeeze(-1)
            ...
            if target_length is not None:
                scale_factor = (target_length / token_num).view(-1, 1)
            ...
            acoustic_embeds, cif_peak, mask_cif = cif(hidden, alphas, self.threshold)
            
            if target_length is None and self.tail_threshold > 0.0:
                token_num_int = torch.max(token_num).type(torch.int32).item()
                acoustic_embeds = acoustic_embeds[:, :token_num_int, :]
                mask_cif = mask_cif[:, :token_num_int]


        return acoustic_embeds, token_num, alphas, cif_peak, mask_cif
...
def cif(hidden, alphas, threshold):
    ...
    for b in range(batch_size):
        ...
        valid_len = l.size(0)
        pad_len = max_label_len - valid_len
        pad_l = torch.zeros([max_label_len - l.size(0), hidden_size], device=hidden.device)
        list_ls.append(torch.cat([l, pad_l], 0))

        valid_mask = torch.cat([
            torch.ones(valid_len, dtype=torch.bool, device=hidden.device),
            torch.zeros(pad_len, dtype=torch.bool, device=hidden.device)
        ], dim=0)
    ...
    return torch.stack(list_ls, 0), fires, mask
