...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_bpe = item["target"]
        if "char" in item and "phone" in item:
            target_bpe=item["target"]
            target_char=item["char"]
            target_phone=item["phone"]
            if self.preprocessor_text:
                target_bpe = self.preprocessor_text(target_bpe)
                target_char = self.preprocessor_text(target_char)
                target_phone = self.preprocessor_text(target_phone)

            if self.tokenizer:
                ids_bpe = self.tokenizer.encode(target_bpe)
                ids_char= self.tokenizer.encode(target_char)
                ids_phone = self.tokenizer.encode(target_phone)

                text_bpe = torch.tensor(ids_bpe, dtype=torch.int64)
                text_char = torch.tensor(ids_char, dtype=torch.int64)
                text_phone = torch.tensor(ids_phone, dtype=torch.int64)
                # text_phone[text_phone == 3] = -1
                # print(text_char)

            else:
                ids_bpe = target_bpe
                ids_char = target_char
                ids_phone = target_phone
                text_bpe = ids_bpe
                text_char = ids_char

                text_phone = ids_phone
                # ids = target
                # text = ids
            ids_bpe_lengths = len(ids_bpe)
            # ids_char_lengths = len(ids_char)
            # ids_phone_lengths = len(ids_phone)

            text_bpe_lengths = torch.tensor([ids_bpe_lengths], dtype=torch.int32)
            # text_char_lengths = torch.tensor([ids_char_lengths], dtype=torch.int32)
            # text_phone_lengths = torch.tensor([ids_phone_lengths], dtype=torch.int32)
        else:
            if self.preprocessor_text:
                target_bpe = self.preprocessor_text(target_bpe)
            ...
            text_char=torch.empty(0)
            # text_char_lengths=torch.empty(0)
            text_phone=torch.empty(0)
            # text_phone_lengths=torch.empty(0)

        return {
                "speech": speech[0, :, :],
                "speech_lengths": speech_lengths,
                "text_bpe": text_bpe,
                "text_bpe_lengths": text_bpe_lengths,
                "text_char": text_char,
                # "text_char_lengths": text_char_lengths,
                "text_phone": text_phone,
                # "text_phone_lengths": text_phone_lengths,
            }
