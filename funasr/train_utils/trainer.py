        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(dataloader_val):
                ...
                if torch.isfinite(loss):
                    total_loss += loss.detach().cpu().item() * weight.sum().item()
                    total_weight += weight.sum().item()
                    batch_count += 1
                    if "acc" in stats:
                        total_acc += stats["acc"].detach().cpu().item() * weight.sum().item()
            ...
            # 在循环后同步并计算全局平均值
            if self.use_ddp or self.use_fsdp:
                total_loss_tensor = torch.tensor(total_loss, dtype=torch.float32).to(self.device)
                total_weight_tensor = torch.tensor(total_weight, dtype=torch.float32).to(self.device)
                total_acc_tensor = torch.tensor(total_acc, dtype=torch.float32).to(self.device)
                batch_count_tensor = torch.tensor(batch_count, dtype=torch.float32).to(self.device)

                dist.all_reduce(total_loss_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(total_weight_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(total_acc_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(batch_count_tensor, op=dist.ReduceOp.SUM)

                total_loss = total_loss_tensor.item()
                total_weight = total_weight_tensor.item()
                total_acc = total_acc_tensor.item()
                batch_count = batch_count_tensor.item()

                if total_weight > 0:
                    self.val_loss_avg = total_loss / total_weight
                    self.val_acc_avg = total_acc / total_weight if total_acc > 0 else 0.0
                else:
                    self.val_loss_avg = 0.0
                    self.val_acc_avg = 0.0
